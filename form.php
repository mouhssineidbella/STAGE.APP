...
<div class="container my-5">
    ...
    <form id="notificationForm" method="POST" action="generate_word.php" class="border border-3 p-4 rounded">
        ...
        <div class="d-grid gap-2">
            <button type="button" onclick="submitForm('generate_word.php')" class="btn btn-primary btn-custom">🔹 تحميل محضر التبليغ</button>
            <button type="button" onclick="submitFilingForm()" class="btn btn-secondary btn-custom">🔹 تحميل طي التبليغ</button>
            <button type="button" onclick="submitCertificateForm()" class="btn btn-success btn-custom">🔹 تحميل شهادة التسليم</button>
        </div>
    </form>
</div>
        ...
        .then(files => {
            ...
            files.forEach((file, index) => {
                setTimeout(() => {
                    ...
                    let fullPath = window.location.origin + "/cours_appel/STAGE.APP/generated_files/" + fileName; // ✅ تحميل من المسار الصحيح
                    console.log("🔹 تحميل الملف: ", fullPath);
